...
async function getListLevels() {
    const response = await axios.get(`https://api.github.com/repos/Abuigsito/gdvzla/contents/public/data/_list-plat.json`, {
        headers: {
...
async function isListUpdatable(db) {
    const now = new Date();
    const doc = await db.collection(COLL_GDVZLA_LIST_CONFIG).findOne({ type: 'lastListUpdateQueryPlat' });

    if (!doc) {
        await db.collection(COLL_GDVZLA_LIST_CONFIG).insertOne({
            type: 'lastListUpdateQueryPlat',
            timeLastUpdate: now
        });
        return true;
    }
...
async function saveSortedList(sha, sortedList) {
    await axios.put(`https://api.github.com/repos/Abuigsito/gdvzla/contents/public/data/_list-plat.json`, {
        message: `Sorted list _list-plat.json`,
...
async function printChangelog(db, client, normalizedLevels, listData) {
    const changelog = await aredlapi.getChangelogPlatformer();
    /** @type {LastChangelog} */
    const doc = await db.collection(COLL_GDVZLA_LIST_CONFIG).findOne({ type: 'lastChangelogPlat' });
